...
export default function RescheduleAppointmentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const res = await fetch(`/api/clinic/appointments/${id}/reschedule`, {
      method: "PATCH",
      headers: {
    ...
    toast({
      title: "Error",
      description: "Unable to reschedule the appointment.",
    });
  } finally {
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                <Avatar className="h-12 w-12">
                  <AvatarImage src={appointment.patient?.image ?? "/user-2.png"} alt={appointment.patient?.name ?? "Patient"} />
                  <AvatarFallback>{appointment?.patient?.name?.charAt(0) ?? "?"}</AvatarFallback>
                </Avatar>
                <div>
                  <h3 className="font-semibold">Patient</h3>
                  <p>{appointment?.patient?.name ?? "Unknown"}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                <Avatar className="h-12 w-12">
                  <AvatarImage src={appointment.doctor?.image ?? "/user-2.png"} alt={appointment.doctor?.name ?? "Doctor"} />
                  <AvatarFallback>{appointment?.doctor?.name?.charAt(0) ?? "?"}</AvatarFallback>
                </Avatar>
                <div>
                  <h3 className="font-semibold">Doctor</h3>
                  <p>{appointment.doctor?.name ?? "Unknown"}</p>
                </div>
              </div>
