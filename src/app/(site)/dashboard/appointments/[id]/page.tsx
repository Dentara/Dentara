...
useEffect(() => {
  async function fetchAppointment() {
    ...
    console.log("🔍 APPOINTMENT DATA:", data); // ✅ BURANI ƏLAVƏ ET
    setAppointment(data);
  }
  fetchAppointment();
}, [id]);
        ...
        <div className="space-y-6">
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center gap-3">
                <Avatar className="h-12 w-12">
                  <AvatarImage src={appointment.patient?.image ?? "/user-2.png"} alt={appointment.patient?.fullName ?? "Unknown"} />
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">DOB: {appointment.patient?.birthDate ? new Date(appointment.patient.birthDate).toLocaleDateString() : "Unknown"}</p>
                </div>
              </div>
              <Separator />
              <div className="space-y-2">
                <div className="grid grid-cols-[20px_1fr] gap-2 items-start">
                  ...
                  <p className="text-sm">{appointment.patient?.address ?? "Unknown"}</p>
                </div>
                <div className="grid grid-cols-[20px_1fr] gap-2 items-center">
                  ...
                  {appointment.patient?.phone ? (
                    <p className="text-sm">{appointment.patient?.phone ?? "Unknown"}</p>
                  ) : (
                ...
                <div className="grid grid-cols-[20px_1fr] gap-2 items-center">
                  ...
                  <p className="text-sm">{appointment.patient?.email ?? "Unknown"}</p>
                </div>
              </div>
              <div className="pt-2">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center gap-3">
                <Avatar className="h-12 w-12">
                  <AvatarImage src={appointment.doctor?.image ?? "/user-2.png"}  alt={appointment.doctor?.fullName ?? "Unknown"} />
                  <AvatarFallback>{appointment.doctor?.fullName?.charAt(0) ?? "?"}</AvatarFallback>
                </Avatar>
                <div>
                  <h3 className="font-semibold">{appointment.doctor?.fullName}</h3>
                  <p className="text-sm text-muted-foreground">{appointment.doctor?.specialty ?? "Not specified"}</p>
                </div>
              </div>
              <Separator />
              <div className="space-y-2">
