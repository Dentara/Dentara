...
export async function GET() {
  ...
  const user = await prisma.user.findUnique({
    where: { id: u.id },
    // createdAt bəzi sxemlərdə yoxdur — seçməyək
  ...
  const treatments = await prisma.treatmentEntry.findMany({
    ...
    select: {
      ...
      attachments: {
        select: { patientFile: { select: { id: true, title: true, path: true } } },
      },
    },
    take: 2000,
    orderBy: { date: "desc" },
  ...
  const res = new NextResponse(body, {
    status: 200,
    headers: {
      // Brauzer məzmunu göstərməsin, birbaşa fayl kimi endirsin
      "Content-Type": "application/octet-stream",
      "Content-Disposition": `attachment; filename="tagiza_export_${new Date()
        .toISOString()
        .slice(0, 10)}.json"`,
    },
  });
