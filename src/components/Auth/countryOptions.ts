// src/components/auth/countryOptions.ts

export type CountryOption = {
  code: string;        // ISO code, e.g. "AZ"
  name: string;        // "Azerbaijan"
  phoneCode: string;   // "+994"
};
...
export function findCountryByCode(code: string | undefined | null): CountryOption | undefined {
  if (!code) return undefined;
  return COUNTRY_OPTIONS.find((c) => c.code === code);
}

export function findCountryByPhoneCode(phoneCode: string | undefined | null): CountryOption | undefined {
  if (!phoneCode) return undefined;
  return COUNTRY_OPTIONS.find((c) => c.phoneCode === phoneCode);
}
